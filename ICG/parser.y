...
expr:       expr OR{push("||");} expr {if($1==1 && $4==1)$$=1; else $$=-1;}
            ...
            | MIMI{push("--"); } expr      {if($3==1)$$=1; else $$=-1;unary();}
            | PLPL{push("++"); } expr      {if($3==1)$$=1; else $$=-1;unary();}
            | expr PLPL{push("++"); }      {if($1==1)$$=1; else $$=-1;unary();}
            | expr MIMI{push("--"); }      {if($1==1)$$=1; else $$=-1;unary();}
...
funcdef:    type ID'('paramlist')'{ fflag=1; strcpy(lastf,$2); strcpy(ty,$1); strcpy(futy,$1); if (cdup($2)) printf("Line %d : Redeclaration of identifer \n",line); else insert($2,"Function",0,1,para,scope,pty,count,0,0);  count=0;int i;memset(para, 0, sizeof para); } stmtblock
            ;
...
forloop:    FOR subroutine '(' expr1 ';' expr1 sub1 ';' expr1 ')' {if($6!=1)printf("Line %d : For expression is not of type INT \n",line-1); }stmt {end();} 
            | FOR subroutine '(' expr1 ';' expr1 sub1';' expr1 ')' {if($6!=1)printf("Line %d : For expression is not of type INT \n",line-1); }stmtblock {end();}
            ;
 
ifstmt:     IF '(' expr sub2 ')' stmtblock elsestmt {if($3!=1)printf("Line %d : If expression is not of type INT \n",line); iflabel();}
            | IF '(' expr sub2 ')' stmt elsestmt {if($3!=1)printf("Line %d : If expression is not of type INT \n",line); iflabel();}
            ;
